...
class JavSpider(scrapy.Spider):
    ...
    allowed_domains = ["javlibrary.com",
                       "torrentkitty.tv"]
    start_urls = ["http://www.javlibrary.com/cn/star_mostfav.php"]
    ...
    def parse(self, response):
        ...
        for yige in road:
            item = JavbusItem()
            item['actress'] = yige.xpath('.//img/@title').extract()[0].encode('utf-8')#.encode('gbk','ignore')
    ...
    def parse_link(self,response):
        ...
        title = sel.xpath('//h3[@class="post-title text"]/a/text()').extract()[0].encode('utf-8')#.encode('gbk','ignore')
        posttime = sel.xpath('//div[@id="video_date"]//td[@class="text"]/text()').extract()[0]
        item2['bango'] = bango
        item2['posttime'] = posttime
        item2['title'] = title
        item2['actress'] = item['actress']
        url = 'http://www.torrentkitty.tv/search/'+bango+'/'
